...
class PackageManager
{
    ...
    {
        $package = $this->get($id);
        // if ($id == 'game-video-poker') {
        //     // dd(file_exists($this->appDirectory . 'packages/' . $package->base_path . '/' . $package->source_path . '/' . str_replace([$package->namespace, '\\'], ['','/'], $package->providers[0]) . '.php'));
        // }
        // dd($package);
    ...
    public function enabled($id): bool
    {
        ...
        // if ($id == 'installer') {
        if ($id == 'game-video-poker') {
            // dd(Storage::disk('local')->exists(sprintf('packages/%s/disabled', $package->id)));
        }
